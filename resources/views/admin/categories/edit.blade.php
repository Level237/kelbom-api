...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Modifier Catégorie</h1>
        <p class="text-sm text-slate-500 mt-1">Mettez à jour les informations de {{ $category->name }}.</p>
    </div>
</div>

<div class="max-w-3xl">
    <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 md:p-8 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $category->name) }}" required class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 @error('name') border-red-500 @enderror">
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $category->description) }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="sort_order" value="{{ old('sort_order', $category->sort_order) }}" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500">
            </div>

            <!-- Icon -->
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">Nom de l'icône (Optionnel)</label>
                <input type="text" name="icon" value="{{ old('icon', $category->icon) }}" placeholder="ex: tag, home, star..." class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500">
            </div>
        </div>
        ...
        <div>
            ...
            @if($category->image_url)
                <div class="mb-4 flex items-center gap-4">
                    <img src="{{ Storage::url($category->image_url) }}" alt="Aperçu" class="w-16 h-16 rounded-lg object-cover bg-slate-100 border border-slate-200">
        ...
        <div class="pt-4 border-t border-slate-100">
            <label class="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" class="w-5 h-5 rounded border-slate-300 text-blue-600 focus:ring-blue-500" {{ old('is_active', $category->is_active) ? 'checked' : '' }}>
